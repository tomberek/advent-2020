            ...
            for y in self.y - 1..self.y + 2 {
                for z in self.z - 1..self.z + 2 {
                    for w in self.w - 1..self.w + 2 {
                        if (x, y, z, w) != (self.x, self.y, self.z, self.w) {
                            neighbors[i] = Cell { x: x, y: y, z:z ,w:w};
                            i += 1;
                        }
                    }
                }
...
impl Field {
    ...
    fn neighbor_counts(&self) -> Counts {
        ...
        let mut neighbors: Neighbors = [Cell { x: 10, y: 10, z:10, w:10 }; 80];
        for cell in &self.0 {
    ...
    fn to_string(&self, f: &mut dyn fmt::Write, padding: i64) -> fmt::Result {
        ...
        let mut minw = i64::max_value();
        let mut maxw = i64::min_value();
